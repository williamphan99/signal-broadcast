    ...
    def test_photo_drag_preview_save_and_lock(self):
        ...
        with tempfile.TemporaryDirectory(prefix="sb-photo-ui-") as directory:
            ...
            try:
                ...
                original_message = app.message.get("1.0", "end-1c")
                app.message_expand.invoke()
                app.activity_expand.invoke()
                app.add_activity("Expanded activity fixture")
                app.update()
                self.assertEqual(int(app.message.cget("height")), 16)
                self.assertEqual(int(app.activity.cget("height")), 16)
                self.assertEqual(app.message.get("1.0", "end-1c"), original_message)
                self.assertIn("Expanded activity fixture", app.activity.get("1.0", "end"))
                self.assertLess(app.send_canvas.yview()[1], 1)
                app.send_canvas.yview_moveto(1)
                app.update()
                self.assertAlmostEqual(app.send_canvas.yview()[1], 1)
                self.assertLessEqual(app.activity.winfo_rooty() + app.activity.winfo_height(),
                                     app.send_canvas.winfo_rooty() + app.send_canvas.winfo_height())
                app.message_expand.invoke()
                app.activity_expand.invoke()
                self.assertEqual(int(app.message.cget("height")), 4)
                self.assertEqual(int(app.activity.cget("height")), 4)
                strip._tips[0].show()
