...
@contextmanager
def _notes_transaction() -> Iterator[None]:
    """Serialize a complete notes read/modify/write across threads and processes."""
    with _NOTES_LOCK:
        NOTES_LOCK_FILE.parent.mkdir(parents=True, exist_ok=True)
        with NOTES_LOCK_FILE.open("a+") as fh:
            fcntl.flock(fh, fcntl.LOCK_EX)
            try:
                yield
            finally:
...
def _note_from_envelope(envelope: dict, media_downloaded: bool) -> dict | None:
    ...
    if not (note["text"] or note["photos"] or note["missing_photos"]
            or note["view_once_photos"] or note.get("missing_body")):
...
def delete_note(timestamp: int) -> None:
    """Delete one stored note without racing a receive in another process."""
    with _notes_transaction():
        kept = [note for note in _read_notes_locked() if note.get("ts") != timestamp]
        _write_notes_locked(kept)
...
class SignalCliDaemon:
    """One long-lived `signal-cli jsonRpc` process for a whole broadcast.

    This avoids the ~1-2s JVM startup per group and keeps encryption sessions warm. It
    holds signal-cli's account lock for its lifetime, so start it only after any other
    signal-cli process has exited."""
